...
/**
 *
 * Klasa bazująca na GlueList - wydajniejsza niż ArrayList i LinkedList
 *
 * @author milosz
 * @param <T>
 */
public class TList<T> extends AbstractList<T> implements List<T>, Cloneable, Serializable {
    ...
    public static <T> TList<T> fromIterable(Iterable<T> elements) {
        return new TList<>(new Iterable[]{elements});
    }

    public TList(Iterable<T>... elements) {
        this();
    ...
    public <E> TList(Callable1<T, E> mapper, E... elements) {
        this();
        if (elements == null || elements.length == 0 || mapper == null)
            return;
        for (E elm : elements) {
            T obj = mapper.run(elm);
            if (obj != null)
                add(obj);
        }
    }

    public <E> TList(Callable1<T, E> mapper, Iterable<E>... elements) {
        this();
        if (elements == null || elements.length == 0 || mapper == null)
            return;
        for (Iterable<E> itr : elements)
            for (E elm : itr) {
                T obj = mapper.run(elm);
                if (obj != null)
                    add(obj);
...
//    }
    ...
    public T first() {
        if (first != null && first.elementData.length > 0)
            return first.elementData[0];
        return null;
    }

    public T last() {
        if (last != null && last.elementDataPointer > 0)
            return last.elementData[last.elementDataPointer - 1];
        return null;
    }
    ...
    public T removeLast() {
        if (last != null && last.elementDataPointer > 0)
            return removeFromNode(last, last.elementDataPointer - 1);
        return null;
    }
    ...
    private T removeFromNode(Node<T> node, int nodeArrIndex) {
    ...
    public T remove(int index) {
        ...
        int nodeArrIndex = index - node.startingIndex;
        return removeFromNode(node, nodeArrIndex);
    }
    ...
    public TList<T> clone() {
        ...
        try {
            TList<T> clone = (TList<T>) super.clone();
    ...
    static class Node<T> {
        ...
        Node(Node<T> pre, Node<T> next, int listSize) {
            ...
            this.elementData = (T[]) new Object[listSize];
