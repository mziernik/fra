 ...
 * enums. The type adapters will write enums in lowercase, despite the fact that
 * they're defined in {@code CONSTANT_CASE} in the corresponding Java model:
 * <pre>   {@code
 *
 ...
 *
 * <p>
 * As with type adapters, factories must be <i>registered</i> with a {@link
 ...
 * }</pre> If multiple factories support the same type, the factory registered
 ...
 *     }
 *   }
 * }</pre> Delegating from one type adapter to another is extremely powerful;
 * it's the foundation of how Gson converts Java objects and collections.
 * Whenever possible your factory should retrieve its delegate type adapter in
 * the {@code create()} method; this ensures potentially-expensive type adapter
