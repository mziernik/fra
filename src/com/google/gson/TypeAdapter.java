 ...
 *       writer.value(xy);
 *     }
 *   }}</pre> With this type adapter installed, Gson will convert {@code Points}
 * to JSON as strings like {@code "5,8"} rather than objects like
 * {@code {"x":5,"y":8}}. In this case the type adapter binds a rich Java class
 * to a compact JSON value.
 *
 * <p>
 ...
 * primitive types this is means readers should make exactly one call to null {@code nextBoolean()}, {@code nextDouble()}, {@code nextInt()}, {@code
...
public abstract class TypeAdapter<T> {
     ...
     *     }
     *   }).create();
     * }</pre> You can avoid this boilerplate handling of nulls by wrapping your
     * type adapter with this method. Here is how we will rewrite the above
     * example:
     * <pre>   {@code
     *
     * Gson gson = new GsonBuilder().registerTypeAdapter(Foo.class,
     ...
     *     }
     *   }.nullSafe()).create();
     * }</pre> Note that we didn't need to check for nulls in our type adapter
     * after we used nullSafe.
     */
     ...
     * this read is strict. Create a {@link JsonReader#setLenient(boolean) lenient}
     ...
     * read is strict. Create a {@link JsonReader#setLenient(boolean) lenient} {@code
