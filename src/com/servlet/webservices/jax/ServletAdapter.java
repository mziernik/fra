 ...
 * specified in the deployment (in particular those information are related to
 * how a request is routed to a {@link ServletAdapter}.
 *
 * <p>
...
public class ServletAdapter extends HttpAdapter implements BoundEndpoint {
     ...
     *
     * @return null if {@link WSEndpoint} isn't tied to any paritcular port.
     */
    ...
    /**
     * Version of {@link #handle(WSHTTPConnection)} that takes convenient
     ...
     * parameters for servlet.
     *
     * Based on the async capabilities of the request and the application
     * processing it, the method may run in asynchronous mode. When run in async
     * mode, this method returns immediately. The response is delayed until the
     * application is ready with the response or the corresponding asynchronous
     * operation times out. The CompletionCallback is guaranteed to run after
     ...
     *
     * @deprecated Use
     * {@link #handle(ServletContext, HttpServletRequest, HttpServletResponse)}
     */
    ...
    private boolean isServlet30Based = isServlet30Based();
