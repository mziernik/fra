...
public class Zip {
    ...
    public static void append(File zipFile, byte[] entry, String entryName,
        ...
        zip.addFile(StrUtils.formatFileName(name), entry, true).comment = fileComment;
        zip.compress(bout, compressionLevel, archiveCommet);

        IOUtils.write(bout.toByteArray(), zipFile);
    ...
    public Zip(InputStream is) throws IOException {
        ...
        try {
            ...
            if (!correct)
                throw new ZipException(LIo.INVALID_ARCHIVE_FORMAT.toString());
        } catch (IllegalArgumentException e) {
            throw new ZipException(LIo.INVALID_ZIP_FILE.toString());
        } finally {
            zis.close();
        }
    }
