...
(function (window, document) {
    ...
    var errHandler = null;
    var docLoadSpinner;
    var vendorProcessed = false;

    window.registerController = function (controllerFunction) {
        if (typeof controllerFunction !== "function")
            throw Error("Argumentem musi być funkcja");
        spa.controllers.push(controllerFunction);
    };

    document.addEventListener("DOMContentLoaded", function (e) {
        docLoadSpinner = new Spinner();
    });

    window.addEventListener("load", function (e) {
        if (e.detail && e.detail.spaLoad)
            return;

        if (spaVendor.error) {
            alert(spaVendor.error);
            return;
        }
        ...
        function waitForLoad(element) {
            ...
            waitForLoadList.push(element);

            element.onload = function (e) {
                var idx = waitForLoadList.indexOf(e.currentTarget);
                if (idx < 0)
                    return;
                waitForLoadList.splice(idx, 1);

                if (vendorProcessed && waitForLoadList.length === 0)
                    try {
                        spa.onAllLoaded();
                    } finally {
                        if (docLoadSpinner)
        ...
        var indexHTML = true;
        for (var name in spaVendor.files || {}) {

            var sourceUrl = "# sourceURL=file:///" + (name.indexOf("/") === 0 ? name.substring(1) : name)

            // gdy wczytani już utils.js
            if (window.ErrorHandler && !errHandler)
                setErrorHandler(new ErrorHandler());

            var src = spaVendor.files[name];

            var ext = name.toLowerCase();
            if (name.indexOf(".") < 0)
                continue;

            ext = ext.substring(name.lastIndexOf(".") + 1);

            if (ext === "css") {
                var tag = document.createElement("style");
                tag.setAttribute("file", name);
                tag.textContent = src + "\n/*" + sourceUrl + "*/";
                waitForLoad(tag);
            ...
            if (ext === "js") {
                ...
                // tag.setAttribute("type", "application/ecmascript;version=6");
                tag.textContent = src + "\n" + "//" + sourceUrl;
                waitForLoad(tag);
                document.head.appendChild(tag);
                files[name] = tag;
            ...
            for (var i = 0; i < html.children.length; i++)
                switch (html.children[i].nodeName) {
                    case "HEAD":
            ...
            if (head)
                for (var j = 0; j < head.children.length; j++) {
                    var src = head.children[j];
                    var tag = document.createElement(src.nodeName);
                    Utils.copyNode(src, tag);
                    waitForLoad(tag);
                    document.head.appendChild(tag);
                }

            files[name] = indexHTML ? document.body
                    : document.createElement(body.nodeName === "BODY" ? "DIV" : body.nodeName);

            indexHTML = false;

            Utils.copyNode(body, files[name]);
        }

        spa.files = files;
        vendorProcessed = true;
        if (waitForLoadList.length === 0)
            try {
                spa.onAllLoaded();
            } finally {
                if (docLoadSpinner)
    ...
    // handler dla nieprzechwyconych wyjątków, ustawiany po zainicjowaniu utils.js
    function setErrorHandler(handler) {
        errHandler = handler;

        errHandler.canDisplay = function (err) {
            return spa.meta.mode === "dev";
        };

        errHandler.sendLog = function (err) {

            if (window.app && window.app.onError) // dla wersji wbudowanej (WebKit JavaFX)
                window.app.onError(err.msg, err.file, err.line, err.column, err);

            if (spa.webApi
                    && spa.webApi.impl
                    && spa.webApi.impl.service
                    && spa.webApi.impl.service.notifications
                    && spa.webApi.impl.service.notifications.log)
                spa.webApi.impl.service.notifications.log({
                    params: {
                        type: "error",
                        value: err.message || ""
    ...
    function compileTS(fileName, data) {
...
// <editor-fold defaultstate="collapsed" desc="=================================== SPINNER ===================================">
(function (window, document) {
    ...
    function build(self) {
        ...
        if (!layer) {
            ...
            showTimeout = setTimeout(function () {
                layer.style.backgroundColor = "rgba(0, 0, 0, 0.2)";
    ...
    body[skin=dark] .spinner > div:before { background-color: #888; }
